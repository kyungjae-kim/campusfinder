...
public class LostServiceClient {
    ...
    public long countByDateRange(LocalDate startDate, LocalDate endDate) {
        try {
            log.debug("Lost 서비스 통계 조회 - startDate: {}, endDate: {}", startDate, endDate);
            ...
            if (response == null || !response.containsKey("count")) {
                log.warn("Lost 서비스 응답이 null이거나 count 필드가 없습니다.");
                return 0L;
            }
            
            Long count = response.get("count");
            log.debug("Lost 서비스 통계 조회 성공 - count: {}", count);
            return count != null ? count : 0L;
            
        } catch (WebClientResponseException e) {
            log.error("Lost 서비스 호출 실패 - Status: {}, Body: {}", 
                e.getStatusCode(), e.getResponseBodyAsString());
            return 0L;
        } catch (Exception e) {
            log.error("Lost 서비스 통계 조회 중 예외 발생", e);
            return 0L;
        }
    }
}
